function doPost(request) {
  ...
  try {
    ...
    if (payload.action !== 'sync_spins' || payload.protocol !== 'spin-record-v2' || !Array.isArray(payload.data)) {
    ...
    var headers = [
      'ID', 'Thời Gian', 'Mã Đại Lý', 'Tên Đại Lý', 'Tỉnh Thành', 'Chủ Đại Lý',
      ...
      'Vòng', 'Vị Trí Trong Vòng', 'a Trước Lượt', 'b Trước Lượt'
    ];
    ...
    payload.data.forEach(function(record) {
      ...
      if (!saved || record.record_version > saved.version) {
        var values = [
          ...
          record.owner_name, record.phone, record.address, record.entry_code, record.serial_number,
    ...
    return ContentService.createTextOutput(JSON.stringify({
      status: 'success', protocol: 'spin-record-v2', acknowledgements: acknowledgements
    })).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({
      status: 'error', protocol: 'spin-record-v2', message: String(error)
    })).setMimeType(ContentService.MimeType.JSON);
  } finally {
