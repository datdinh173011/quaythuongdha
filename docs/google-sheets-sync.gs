  ...
  try {
    ...
    if (existing.length) {
      existing[0].forEach(function(header, index) {
        var legacyHeader = index < headers.length && ['ID', 'Thời Gian', 'Mã Đại Lý', 'Tên Đại Lý', 'Tỉnh Thành', 'Tên Ngân Hàng', 'Số Tài Khoản Ngân Hàng', 'Tên Chủ Tài Khoản Ngân Hàng', 'Số Điện Thoại', 'Địa Chỉ', 'Mã Dự Thưởng', 'Kỳ Thưởng', 'Lượt Trong Kỳ', 'Chủ Đại Lý'].indexOf(header) !== -1;
    ...
    for (var rowIndex = 1; rowIndex < existing.length; rowIndex++) {
      ...
      var isVoid = typeof existing[rowIndex][9] === 'string' && existing[rowIndex][9].indexOf('[ĐÃ HỦY]') === 0;
    ...
    payload.data.forEach(function(record) {
      ...
      if (shouldUpdate) {
        ...
        var values = [
          record.id, record.spin_time, record.agency_code,
          record.bank_name || '', record.bank_account_number || '', record.bank_account_holder_name || '',
        ...
        sheet.getRange(destination, 5, 1, 1).setNumberFormat('@');
        sheet.getRange(destination, 7, 1, 1).setNumberFormat('@');
        sheet.getRange(destination, 9, 1, 1).setNumberFormat('@');
